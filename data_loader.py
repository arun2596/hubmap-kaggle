...
class DatasetRetriever(Dataset):
    def __init__(self, data, mode='train', transform=None):
        ...
        if self.mode == 'train' or self.mode == 'valid':
            self.data_dir = TRAIN_IMAGES_DIR_1024
        elif self.mode == 'test':
            self.data_dir = TEST_IMAGES_DIR
        else:
            raise Exception("Invalid mode: " + str(self.mode))

        self.mask_data_dir = TRAIN_MASK_DIR_1024
    ...
    def __getitem__(self, item):
        ...
        if random.random()<stain_prob and self.mode=='train':
            stain_flag=True
            img_name = os.path.join(STAINED_IMAGES_DIR_1024, str(img_id) + '.tiff')
        else:
...
def make_loader(
    ...
    transform = {'train': transforms.Compose([
        ...
        AlbuDownScale(0.2, input_shape=input_shape),
        AlbuAngleRotate(0.6),
        AlbuElastic(0.4),
