...
class ParallelBlock(nn.Module):
    ...
    def __init__(self, dims, num_heads, mlp_ratios=[], qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        assert dims[1] == dims[2] == dims[3] == dims[4]
        assert mlp_ratios[1] == mlp_ratios[2] == mlp_ratios[3] == mlp_ratios[4]
        mlp_hidden_dim = int(dims[1] * mlp_ratios[1])
        self.mlp2 = self.mlp3 = self.mlp4 = self.mlp5 = Mlp(
    ...
    def forward(self, x1, x2, x3, x4, x5, sizes):
        ...
        downsample2_5 = self.downsample(
            cur2, output_size=(H5, W5), size=(H2, W2))
        downsample3_5 = self.downsample(
            cur3, output_size=(H5, W5), size=(H3, W3))
        downsample4_5 = self.downsample(
            cur4, output_size=(H5, W5), size=(H4, W4))
        ...
        cur5 = cur5 + downsample4_5 + downsample3_5 + downsample2_5
...
class CoaT(nn.Module):
    ...
    def __init__(self, patch_size=16, in_chans=3, embed_dims=[0, 0, 0, 0],
        ...
        self.patch_embed5 = PatchEmbed(
            patch_size=2, in_chans=embed_dims[3], embed_dim=embed_dims[4])
        ...
        self.crpe5 = ConvRelPosEnc(
            Ch=embed_dims[4] // num_heads, h=num_heads, window=crpe_window)
            ...
            self.parallel_blocks = nn.ModuleList([
                ParallelBlock(
                    ...
                    shared_crpes=[self.crpe1, self.crpe2,
                                  self.crpe3, self.crpe4, self.crpe5]
                )
    ...
    def forward(self, x0):
        ...
        # torch.zeros_like(self.cls_token1)#self.cls_token1
        cls = self.cls_token1
        ...
        # torch.zeros_like(self.cls_token2)#self.cls_token2#
        cls = self.cls_token2
        ...
        # torch.zeros_like(self.cls_token3)# self.cls_token3
        cls = self.cls_token3
        ...
        # torch.zeros_like(self.cls_token4)#self.cls_token4
        cls = self.cls_token4
        ...
        x5_nocls = x5_nocls.reshape(
            B, H5, W5, -1).permute(0, 3, 1, 2).contiguous()
        ...
        for blk in self.parallel_blocks:
            x1, x2, x3, x4, x5 = blk(x1, x2, x3, x4, x5, sizes=[
    ...
    # Return intermediate features for down-stream tasks (e.g. Deformable DETR and Detectron2).
        if self.return_interm_layers:
            ...
            x5_cls = x5[:, :1]
            # Shape: [B, 3, C].
            merged_cls = torch.cat((x2_cls, x3_cls, x4_cls, x5_cls), dim=1)
            # Shape: [B, C].
...
def coat_tiny(**kwargs):
    model = CoaT(patch_size=4, embed_dims=[152, 152, 152, 152], serial_depths=[
                 2, 2, 2, 2], parallel_depth=6, num_heads=8, mlp_ratios=[4, 4, 4, 4], **kwargs)
    model.default_cfg = _cfg_coat()
    return model


def coat_mini(**kwargs):
    model = CoaT(patch_size=4, embed_dims=[152, 216, 216, 216], serial_depths=[
                 2, 2, 2, 2], parallel_depth=6, num_heads=8, mlp_ratios=[4, 4, 4, 4], **kwargs)
    model.default_cfg = _cfg_coat()
    return model
...
def coat_small(**kwargs):
    model = CoaT(patch_size=4, embed_dims=[152, 320, 320, 320, 320], serial_depths=[
                 2, 2, 2, 2, 2], parallel_depth=6, num_heads=8, mlp_ratios=[4, 4, 4, 4, 4], **kwargs)
    model.default_cfg = _cfg_coat()
    return model
...
def coat_lite_tiny(**kwargs):
    model = CoaT(patch_size=4, embed_dims=[64, 128, 256, 320], serial_depths=[
                 2, 2, 2, 2], parallel_depth=0, num_heads=8, mlp_ratios=[8, 8, 4, 4], **kwargs)
    model.default_cfg = _cfg_coat()
    return model


def coat_lite_mini(**kwargs):
    model = CoaT(patch_size=4, embed_dims=[64, 128, 320, 512], serial_depths=[
                 2, 2, 2, 2], parallel_depth=0, num_heads=8, mlp_ratios=[8, 8, 4, 4], **kwargs)
    model.default_cfg = _cfg_coat()
    return model
...
def coat_lite_small(**kwargs):
    model = CoaT(patch_size=4, embed_dims=[64, 128, 320, 512], serial_depths=[
                 3, 4, 6, 3], parallel_depth=0, num_heads=8, mlp_ratios=[8, 8, 4, 4], **kwargs)
    model.default_cfg = _cfg_coat()
    return model


def coat_lite_medium(**kwargs):
    model = CoaT(patch_size=4, embed_dims=[128, 256, 320, 512, 640], serial_depths=[
                 3, 6, 10, 8, 6], num_heads=8, parallel_depth=0, mlp_ratios=[4, 4, 4, 4, 4], **kwargs)
    model.default_cfg = _cfg_coat()
    return model
